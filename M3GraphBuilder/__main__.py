...
def main():
    parser = argparse.ArgumentParser(description="A CLI application for extracting a graph out of C3 model.")
    parser.add_argument("-m_path", "--model_path", type=str, required=True, help="the path of the C3 model (Required)")
    parser.add_argument("-g_name", "--graph_name", type=str, required=False, help="the name of the generated graph (Optional)")
    parser.add_argument("-f_path", "--folder_path", type=str, required=False, help="the path to the folder in which to save the graph (Optional)")
    parser.add_argument("-f", "--full", action="store_true", help="Extracts nodes describing variables, primitives and function parameters.")
    parser.add_argument("-v", "--validate", action="store_true", help="Validates whether the nodes in the generated graph, and the ones in the edges are consistent.")
